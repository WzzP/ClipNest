...
版本由仓库根目录 `VERSION` 管理，当前为 `0.1.3`，对应 Git tag `v0.1.3`。Release 是否标记为正式版，与是否通过 Apple 签名/公证是两个不同维度；未公证包必须明确披露。
...
```text
.build/releases/0.1.3/local/
├── ClipNest.app
├── ClipNest-0.1.3-universal-unsigned.zip
└── SHA256SUMS.txt
```
...
结果位于 `.build/releases/0.1.3/notarized/`。只上传最终 `ClipNest-0.1.3-universal.zip` 与 `SHA256SUMS.txt`；不要上传中间的 `notary-upload.zip` 或公证日志。签名、公证任何一步失败，脚本都会停止。
...
```sh
git tag -a v0.1.3 -m "ClipNest 0.1.3"
git push origin v0.1.3
```

在 GitHub 仓库 **Releases → Draft a new release** 中：

- 选择 `v0.1.3`，标题使用 `ClipNest 0.1.3`。
- 说明使用 `docs/releases/v0.1.3.md`。
- 上传 ZIP 与 `SHA256SUMS.txt`。
- 作为正式 0.1.3 版本发布时，不勾选 Pre-release。
...
gh release create v0.1.3 \
  .build/releases/0.1.3/local/ClipNest-0.1.3-universal-unsigned.zip \
  .build/releases/0.1.3/local/SHA256SUMS.txt \
  --repo WzzP/ClipNest --verify-tag --draft \
  --title "ClipNest 0.1.3" --notes-file docs/releases/v0.1.3.md
```
